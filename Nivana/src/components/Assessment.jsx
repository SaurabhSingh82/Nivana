...
// --- 1. QuestionCard Moved OUTSIDE the main component (FIXED) ---
const QuestionCard = ({ 
  q, 
  answers, 
  setAnswer, 
  ...
  // Sync local text state with answers when question changes
  useEffect(() => {
    if (q?.type === "text") {
...
export default function Assessment() {
  ...
  const submitAssessment = async () => {
    if (!auth.isAuthenticated) {
      setToast({ type: 'error', text: 'Please log in to submit the assessment.' });
      setTimeout(() => setToast(null), 3500);
      setTimeout(() => navigate('/login', { state: { next: '/dashboard' } }), 900);
      return;
    }
    ...
    try {
      const data = await apiService.submitAssessment({ age: Number(age), answers: payloadAnswers });

      const saved = data.assessment || data;
      const analysis = saved.llmAnalysis || saved.llm_analysis || null;
      if (analysis && (analysis.guidanceText || analysis.guidance)) {
        setLlmResult({ text: analysis.guidanceText || analysis.guidance, severity: analysis.severity, risks: analysis.risks || [] });
      } else {
        setToast({ type: "success", text: "Assessment saved. Check dashboard." });
        setTimeout(() => setToast(null), 3000);
      }
      ...
      if (data.emergency || saved.emergency) {
        ...
        setTimeout(() => navigate("/dashboard", { state: { savedAssessment: saved } }), 900);
      }
      ...
      if (err.response?.status === 401) {
        ...
        setTimeout(() => setToast(null), 3500);
        setTimeout(() => navigate('/login', { state: { next: '/assessments' } }), 900);
      } else {
        setToast({ type: "error", text: err.response?.data?.msg || "Submission failed" });
        setTimeout(() => setToast(null), 3000);
      }
            ...
            <div className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  min="8" max="120" 
                  placeholder="e.g. 24" 
                  className="w-full p-5 rounded-2xl bg-gray-50 border border-gray-100 focus:bg-white focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all text-2xl font-semibold text-gray-800 placeholder-gray-300" 
                  value={age} 
              ...
              <div className="min-h-[60vh] flex flex-col">
                {step < questions.length && (
                   <AnimatePresence mode="wait">
                      ...
                      >
                         {/* ✅ FIXED: Passing props instead of using closure */}
                         <QuestionCard 
                            q={questions[step]} 
                            answers={answers}
                            setAnswer={setAnswer}
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
                      ...
                      <button onClick={submitAssessment} disabled={submitting} className={`w-full sm:flex-1 py-4 rounded-xl text-white font-bold text-lg shadow-xl transition-all transform hover:-translate-y-1
                        ${submitting ? 'bg-gray-300' : 'bg-gradient-to-r from-green-600 to-teal-600 shadow-green-200'}`}>
        ...
        {toast && (
          <div className="fixed top-24 left-1/2 transform -translate-x-1/2 z-50">
            <motion.div 
               ...
               animate={{ opacity: 1, y: 0 }} 
               exit={{ opacity: 0 }}
               className={`px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border ${toast.type === 'error' ? 'bg-white border-red-100 text-red-600' : 'bg-gray-900 text-white border-gray-800'}`}
            >
              <div className="text-lg">{toast.type === 'error' ? '⚠️' : '✅'}</div>
